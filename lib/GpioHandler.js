...
class GpioHandler {
  ...
  async init () {
    ...
    this._gpio.events.on(eventNames.ERROR, this._traceGpioError.bind(this))
  }

  _traceGpioError (error) {
    this._module.tracer.error(`Error event received from gpio ${this._gpioNumber}`, error)
  }
